class Bird{
  ...
  public void repel(PVector repelPt)
  {
    
    PVector repelPoint = repelPt;
    float distance = PVector.dist(position, repelPoint);
    
    if (distance > 0 && distance < searchDistance)
    {
      PVector desiredVec = PVector.sub(repelPoint, position);
      desiredVec.normalize();
      desiredVec.mult(maxSpeed);
    
      PVector steerVec = PVector.sub(desiredVec, velocity);
      steerVec.limit(maxSteeringForce);
      steerVec.mult(-repelWeight);
      applyForce(steerVec);
    }
    else {
